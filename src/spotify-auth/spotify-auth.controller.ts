...
export class SpotifyAuthController {
  ...
  async callback(
    ...
    if (error) {
      if (isMobile) {
        return res.redirect(
          `com.enterlist.app://oauth/error?error=${encodeURIComponent(error)}&provider=spotify`,
        );
      }
      return res.redirect(
    ...
    try {
      ...
      if (isMobile) {
        return res.redirect(
          `com.enterlist.app://oauth/error?error=${encodeURIComponent(errorMessage)}&provider=spotify`,
        );
      }
  ...
  async getPlaylists(
    ...
    return this.spotifyAuthService.getUserPlaylists(
      req.user.user_id,
      limit,
      offset,
    );
  ...
  async getUserTracks(
    ...
    return this.spotifyAuthService.getUserTracks(
      req.user.user_id,
      limit,
      offset,
    );
  ...
  async importPlaylists(@Req() req, @Body() body: { playlistIds: string[] }) {
    return this.spotifyAuthService.importPlaylistsToDatabase(
      req.user.user_id,
      body.playlistIds,
    );
  }
  @Post('import/tracks')
  @UseGuards(JwtAuthGuard, EmailConfirmedGuard, RoleRequiredGuard)
  async importTracks(@Req() req, @Body() body: { trackIds: string[] }) {
    return this.spotifyAuthService.importTracksToDatabase(
      req.user.user_id,
      body.trackIds,
    );
  }
