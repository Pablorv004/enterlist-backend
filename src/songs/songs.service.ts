  ...
  async create(createSongDto: CreateSongDto) {
    ...
    if (existingSong) {
      throw new ConflictException(
        `Song already exists for this platform and ID`,
      );
    }
  ...
  async syncSongs(artistId: string): Promise<any> {
    ...
    if (results.spotify) {
      platformResults.push(
        `Spotify: ${results.spotify.updated?.length || 0} updated`,
      );
    }
    if (results.youtube) {
      platformResults.push(
        `YouTube: ${results.youtube.updated?.length || 0} updated`,
      );
    }
