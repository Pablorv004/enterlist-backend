...
export class PaypalAuthService {
  ...
  constructor(
    ...
    if (!this.clientId || !this.clientSecret) {
      ...
      this.logger.log(
        `PayPal OAuth service initialized in ${this.environment} mode`,
      );
  ...
  async getAuthorizationUrl(
    ...
    const scopes = ['openid', 'email'].join(' ');
    ...
    authUrl.searchParams.set(
      'redirect_uri',
      isMobile ? this.mobileRedirectUri : this.redirectUri,
    );
    ...
    this.logger.log(
      `Generated PayPal OAuth URL for ${userId ? 'existing user' : 'new user'}`,
    );
  ...
  async handleCallback(code: string, state: string): Promise<any> {
    ...
    const cleanEmail =
      profile.email || `${profile.user_id.split('/').pop()}@paypal.user`;
    const cleanName =
      profile.name ||
      profile.given_name ||
      profile.family_name ||
  ...
  private async exchangeCodeForTokens(code: string): Promise<any> {
    try {
      ...
      this.logger.log(
        'Successfully exchanged PayPal authorization code for tokens',
      );
  ...
  async createPayment(
    ...
    try {
      ...
      this.logger.log(
        `Creating PayPal payment for amount: ${(amount / 100).toFixed(2)} ${currency}`,
      );
      ...
      if (error.response?.data?.name === 'DUPLICATE_TRANSACTION') {
        throw new BadRequestException(
          'A duplicate PayPal transaction was detected. Please try again with a different submission.',
        );
      } else if (error.response?.status === 429) {
        throw new BadRequestException(
          'Too many PayPal requests. Please wait a moment and try again.',
        );
      }
  ...
  async createPayout(
    ...
    try {
      ...
      this.logger.log(
        `PayPal payout created: ${response.data.batch_header.payout_batch_id}`,
      );
      return response.data;
    } catch (error) {
      this.logger.error(
    ...
    try {
      ...
      if (validToken) {
        try {
          ...
          this.logger.error(
            'Failed to get PayPal email from token:',
            tokenError,
          );
          throw new UnauthorizedException(
            'PayPal account needs to be re-linked',
          );
        }
      } else {
  ...
  async getValidUserAccessToken(userId: string): Promise<string | null> {
    ...
    if (!tokenExpiresAt || now >= tokenExpiresAt) {
      this.logger.log(
        `PayPal token expired for user ${userId}, attempting to refresh`,
      );
      // Token is expired, try to refresh
      if (paymentMethod.refresh_token) {
        try {
          const refreshedTokens = await this.refreshAccessToken(
            paymentMethod.refresh_token,
          );
          ...
          if (refreshedTokens.expires_in) {
            newExpiresAt.setSeconds(
              newExpiresAt.getSeconds() + refreshedTokens.expires_in,
            );
          } else {
          ...
          await this.prismaService.paymentMethod.update({
            ...
            data: {
              ...
              refresh_token:
                refreshedTokens.refresh_token || paymentMethod.refresh_token,
          ...
          this.logger.log(
            `Successfully refreshed PayPal access token for user ${userId}, expires at ${newExpiresAt.toISOString()}`,
          );
          return refreshedTokens.access_token;
        } catch (error) {
          this.logger.error(
            `Failed to refresh PayPal token for user ${userId}:`,
            error,
          );
          return null;
        }
        ...
        this.logger.warn(
          `No refresh token available for user ${userId}, re-authentication required`,
        );
        return null;
      }
    }
    ...
    this.logger.log(
      `Using existing valid PayPal token for user ${userId}, expires at ${tokenExpiresAt.toISOString()}`,
    );
  ...
  private async refreshAccessToken(refreshToken: string): Promise<any> {
    try {
      ...
      if (response.data.expires_in) {
        this.logger.log(
          `New token expires in ${response.data.expires_in} seconds`,
        );
      }
      return response.data;
    } catch (error) {
